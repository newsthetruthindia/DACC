...
export function CardHeader({ children, className = '' }) {
  return <div className={`px-6 py-4 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/40 ${className}`}>{children}</div>;
}
  ...
  return (
    <Card className="hover:border-zinc-700 transition-all">
      <CardBody className="p-5">
        ...
        <div className="text-3xl font-extrabold tracking-tight text-white mb-2" style={{ color }}>{value}</div>
...
export function Badge({ label }) {
  return (
    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${badgeMap[label] || 'bg-zinc-800 text-zinc-400 border border-zinc-700'}`}>
      {label}
    </span>
  );
}
...
export function Btn({ children, onClick, variant = 'primary', size = 'md', disabled, className = '', type = 'button' }) {
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled}
      className={`inline-flex items-center justify-center gap-2 font-semibold transition-all ${vars[variant] || vars.primary} ${sizes[size]} ${disabled ? 'opacity-40 cursor-not-allowed' : ''} ${className}`}>
      {children}
    </button>
  );
}
...
export function Input({ label, ...props }) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input className="px-4 py-3 border border-zinc-700 rounded-xl text-sm text-white bg-[#1a1a22] outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all placeholder:text-zinc-500" {...props} />
    </div>
  );
}
...
export function Textarea({ label, ...props }) {
  return (
    <div className="flex flex-col gap-1.5">
      {label && <label className="text-xs font-semibold text-zinc-300">{label}</label>}
      <textarea className="px-4 py-3 border border-zinc-700 rounded-xl text-sm text-white bg-[#1a1a22] outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all resize-none placeholder:text-zinc-500" rows={4} {...props} />
    </div>
  );
}
...
export function Select({ label, children, ...props }) {
  return (
    <div className="flex flex-col gap-1.5">
      {label && <label className="text-xs font-semibold text-zinc-300">{label}</label>}
      <select className="px-4 py-3 border border-zinc-700 rounded-xl text-sm text-white bg-[#1a1a22] outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all font-medium" {...props}>
        {children}
      </select>
    </div>
  );
...
export function ToastProvider() {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 px-5 py-3.5 rounded-xl text-sm font-semibold shadow-2xl flex items-center gap-3 animate-fade-in border ${
...
export function UpiModal({ open, onClose, link, amount, month, memberName, onMarkPaid }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-[#131318] border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <div className="p-6">
          <div className="grid grid-cols-2 gap-3 mb-5">
            {apps.map(app => (
              <a key={app.name} href={link} onClick={onMarkPaid}
                className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-center hover:border-orange-500 hover:bg-zinc-800 transition-all block no-underline">
          ...
          <div className="bg-black/40 border border-zinc-800 rounded-xl p-3 font-mono text-xs text-zinc-300 break-all mb-6 select-all">
            {link}
          </div>
