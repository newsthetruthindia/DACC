...
export default function DashboardPage() {
  ...
  return (
    <AppLayout>
      {/* Welcome Header */}
      <div className="mb-8 p-6 lg:p-8 rounded-2xl bg-[#131318] border border-zinc-800 shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
      ...
      {isPanel && summary && (
        <div className="mb-8">
          <h2 className="text-sm font-bold uppercase tracking-wider text-zinc-400 mb-4">Committee Overview ({month})</h2>
      ...
      {/* Member Stats Grid */}
      <h2 className="text-sm font-bold uppercase tracking-wider text-zinc-400 mb-4">My Membership Status</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <a href="/notifications" className="text-xs font-semibold text-orange-400 hover:underline">View All →</a>
          </CardHeader>
          {notifs.length === 0
            ? <Empty icon="📭" title="No announcements posted yet" />
            : <div className="divide-y divide-zinc-800">
                {notifs.map(n => (
                  <div key={n._id} className={`p-5 transition-colors hover:bg-zinc-900/50 ${!n.isRead ? 'bg-orange-500/5 border-l-2 border-orange-500' : ''}`}>
                    <div className="flex items-center gap-2 mb-1">
                      {!n.isRead && <span className="w-2 h-2 rounded-full bg-orange-500"></span>}
        ...
        <Card>
          <CardHeader>
            ...
            <a href="/panel" className="text-xs font-semibold text-orange-400 hover:underline">View All →</a>
          </CardHeader>
          <CardBody>
            {panel.length === 0
                    ...
                    return (
                      <div key={p._id} className="flex items-center gap-3 p-3.5 bg-zinc-900/60 border border-zinc-800 rounded-xl">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="text-xs text-orange-400 font-medium mt-0.5">{p.panelRole}</div>
                        </div>
                      </div>
                    );
                  })}
